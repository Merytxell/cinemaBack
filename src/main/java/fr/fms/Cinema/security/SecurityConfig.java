...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
        ...
        protected void configure(HttpSecurity http) throws Exception{
            http.csrf().disable().cors().configurationSource(request -> new CorsConfiguration().applyPermitDefaultValues())
                    ...
                    .antMatchers(HttpMethod.POST, "/cinemas").authenticated()
                    .antMatchers(HttpMethod.POST, "/cinemas").hasAuthority("ADMIN")
                    .antMatchers(HttpMethod.POST, "/update/{id}").hasAuthority("ADMIN")
                    .antMatchers(HttpMethod.DELETE, "/movies/{id}").hasAuthority("ADMIN")
